...
func staticMitigation() {
	...
	for i := 0; i < CONFIGURATION.INGRESS_LOC; i++ {
		...
		perSec_tcp := ((AVG_TRAFFIC[i] / CONFIGURATION.EPOCH_TIME) * PKT_LEN)/CURR_TRAFFIC_STATS[i]["tcp_syn"]
		perSec_udp := ((AVG_TRAFFIC[i] / CONFIGURATION.EPOCH_TIME) * PKT_LEN)/CURR_TRAFFIC_STATS[i]["udp_flood"]
