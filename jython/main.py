...
class RepeatingThread(Thread):
    ...
    def run(self):
        globals.DEBUG_LOGGER.debug("Hello from thread %s",self.threadName)
        globals.DEBUG_LOGGER.debug("Thread repeat time = %s", self.waitiingTime)
...
def readConfigureFile(file):
	...
	globals.DEBUG_LOGGER.debug("Processing delay = %s seconds", str(globals.PROCESSING_DELAY) )
