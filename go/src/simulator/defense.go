...
func initializeDefense(){
	...
	for i := 0 ; i < CONFIGURATION.INGRESS_LOC ; i++ {
		...
		dequeuePkts := int(math.Floor((float64(CONFIGURATION.NUM_NIC_VM*NUM_VMs[i])*CONFIGURATION.VM_COMPUTE_CAP)/PKT_LEN))
		...
		_DEBUG.Printf("Function: initializeAdaptive - Initial queue size at ingress %d = %f", i ,dequeuePkts)
